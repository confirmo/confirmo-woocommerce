 ...
 *
 * So the suite needs a WordPress with WooCommerce and WooCommerce Subscriptions
 * active. `demo/` provides one; `WP_ROOT` points at any other.
 *
...
foreach (['WooCommerce' => 'WooCommerce', 'WC_Subscriptions' => 'WooCommerce Subscriptions'] as $class => $name) {
    if (!class_exists($class)) {
        fwrite(STDERR, "{$name} is not active on the WordPress at {$wpRoot}.\n");
        exit(1);
    }
}
